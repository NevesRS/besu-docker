#!/bin/bash
echo "Aguardando RPC estar disponível..."
...
curl -X POST --data "$ALLOWLIST_JSON" -H "Content-Type: application/json" http://127.0.0.1:8545/
sleep 2  # Permissões aplicadas

# Adicionar conexões entre todos os nós
curl -X POST --data '{"jsonrpc":"2.0","method":"admin_addPeer","params":["'$NODE2'"],"id":1}' -H "Content-Type: application/json" http://127.0.0.1:8545
curl -X POST --data '{"jsonrpc":"2.0","method":"admin_addPeer","params":["'$NODE3'"],"id":1}' -H "Content-Type: application/json" http://127.0.0.1:8545
curl -X POST --data '{"jsonrpc":"2.0","method":"admin_addPeer","params":["'$NODE4'"],"id":1}' -H "Content-Type: application/json" http://127.0.0.1:8545
